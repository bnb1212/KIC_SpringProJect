...
$( document ).ready(function(){
	...
	$('#insertbtn').click(function(){ //댓글 등록 버튼 클릭시 
		if($('#detplace').val() == ""){
			...
			alert(vno)
			var mno = $('#dmno').val();
		    dapdet_insert(content,vno,mno); //아래 만들어 놓은 Insert 함수를 호출
		}
	});
...
function dapdet_update(det_no,content,vno){
    ...
    //a += '<input type="hidden" class="form-control" id="content_'+det_no+'"/>';
    a += '<input type="button" value="수정" onclick="dapdet_updateProc('+det_no+',\''+vno+'\')">'; 
    a += '</form></div>';
    
    $('.dapdetInfo'+det_no).html(b);
...
function showdap(param){
    ...
    aa += '<td><textarea id="dapdetplace" name="dapdetplace" cols="120" rows="2" placeholder="답글을 입력해주세요" style=" margin-right: 10px;"></textarea></td>';
...
<table>
<tr>
<td><i class="far fa-smile-wink" style="font-size:50px; margin-right: 10px;"></i></td>
<td><textarea id="detplace" name="detplace" cols="120" rows="2" placeholder="댓글을 입력해주세요" style=" margin-right: 10px;"></textarea></td>
